...
vector_t *vector_new() {
	...
	//valgrind --leak-check=full ./vector-test	/* and return... */
	return retval;
}
...
//	v attributes: 	data - which w/ zero offset pts to beginning of array
//			size - size of each element of array
//	loc : where within the vector you want to place value
//	value : int what you want to place within vector
void vector_set(vector_t *v, size_t loc, int value) {
	/* What do you need to do if the location is greater than the size we have
	 * allocated?  Remember that unset locations should contain a value of 0.
	 */

    //reallocate memory to correct size
    if (loc > v->size)
    {
