	...
	switch (inst.rtype.opcode) /* could also use e.g. inst.itype.opcode */
	{
			...
			switch (inst.rtype.funct)
		{
			case 0x0: // funct == 0x0 (sll)
				p->R[inst.rtype.rd] =  p->R[inst.rtype.rt] << R[inst.rtype.shamt];
				p->pc += 4;
				break;
			...
			case 0x3: // funct == 0x3 (sra)
				p->R[inst.rtype.rd] =  p->(int32_t)(int8_t)R[inst.rtype.rt] >> R[inst.rtype.shamt] ;
				p->pc += 4;
				break;
			...
			case 0x2a: // funct == 0x2a (slt)
				p->R[inst.rtype.rd] = (int32_t)(int8_t)p->R[inst.rtype.rs] < (int32_t)(int8_t)p->R[inst.rtype.rt]);
				p->pc += 4;
				break;
		...
		case 0x5: // opcode == 0x5 (bne)
			if (*(p->R[inst.rtype.rs]) != *(p->R[inst.rtype.rt])) {
				p->pc += (4 + signext((int16_t)inst.itype.imm)*4);
			}
			break;
			// may need to look at this
		case 0x9: // opcode == 0x9 (addiu)
			p->R[inst.rtype.rt] = p->R[inst.rtype.rs] + signext((int16_t)inst.itype.imm);
			p->pc += 4;
			break;
			
		case 0xa: // opcode == 0xa (slti)
			p->R[inst.rtype.rt] = (signed)(p->R[inst.rtype.rs]) < signext((int16_t)inst.itype.imm);
			p->pc += 4;
			break;
		...
		case 0xe: // opcode == 0xe (xori)
			p->R[inst.rtype.rt] = p->R[inst.rtype.rs] ^ zeroext(inst.itype.imm);
			p->pc += 4;
			break;
			
		case 0xf: // opcode == 0xf (lui)
			p->R[inst.rtype.rt] = chunks16[1](inst.itype.imm << 16);
			p->pc += 4;
			break;
			// may need to look at this
		case 0x20: // opcode == 0x20 (lb)
			p->R[inst.rtype.rt] = signext(LoadMem(p->R[inst.rtype.rt]+signext((int16_t)inst.itype.imm), byte));
			p->pc += 4;
			break;
			
		case 0x21: // opcode == 0x21 (lh)
			p->R[inst.rtype.rt] = signext(LoadMem(p->R[inst.rtype.rt]+signext((int16_t)inst.itype.imm), half));
			p->pc += 4;
			break;
			
		case 0x23: // opcode == 0x23 (lw)
			p->R[inst.rtype.rt] = LoadMem(p->R[inst.rtype.rt]+signext((int16_t)inst.itype.imm), word);
			p->pc += 4;
			break;
			
		case 0x24: // opcode == 0x24 (lbu)
			p->R[inst.rtype.rt] = zeroext(LoadMem(p->R[inst.rtype.rs]+...
												  signext((int16_t)inst.itype.imm),byte));
			p->pc += 4;
			break;
			
		case 0x25: // opcode == 0x25 (lhu)
			p->R[inst.rtype.rt] = zeroext(LoadMem(p->R[inst.rtype.rs]+...
												  signext((int16_t)inst.itype.imm), half));
			p->pc += 4;
			break;
			
		case 0x28: // opcode == 0x28 (sb)
			StoreMem(p->R[inst.rtype.rs]+signext((int16_t)inst.itype.imm), byte, p->R[inst.rtype.rt]);
			p->pc += 4;
			break;
			
		case 0x29: // opcode == 0x29 (sh)
			StoreMem(p->R[inst.rtype.rs]+signext((int16_t)inst.itype.imm), half, p->R[inst.rtype.rt]);
			p->pc += 4;
			break;
			
		case 0x2b: // opcode == 0x2b (sw)
			StoreMem(p->R[inst.rtype.rs]+signext((int16_t)inst.itype.imm), word, p->R[inst.rtype.rt]);
			p->pc += 4;
			break;			
