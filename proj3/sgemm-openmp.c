    ...
    {   
        ...
        for (j = 0; j < m; j+=JROLL) {  
            for (i = 0; i < roll_limit; i+=ROWWIDTH) {
				...
				/*__m128 localsum1b = _mm_setzero_ps();
                __m128 localsum2b = _mm_setzero_ps();
                __m128 localsum3b = _mm_setzero_ps();
                __m128 localsum4b = _mm_setzero_ps();
                /*__m128 localsum1c = _mm_setzero_ps();
                __m128 localsum2c = _mm_setzero_ps();
                __m128 localsum3c = _mm_setzero_ps();
                __m128 localsum4c =  _mm_setzero_ps();
				 */
				#pragma omp unroll
                for(k = 0; k < kroll_limit; k+=4) {
					...
					/*
					localsum1b = _mm_add_ps(localsum1b, _mm_mul_ps(*(__m128*)(padtrans+k+i*nRow), _mm_loadu_ps(padtrans+k+(j+2)*nRow)));
                    localsum2b = _mm_add_ps(localsum2b, _mm_mul_ps(*(__m128*)(padtrans+k+(i+1)*nRow), _mm_loadu_ps(padtrans+k+(j+2)*nRow)));
                    localsum3b = _mm_add_ps(localsum3b, _mm_mul_ps(*(__m128*)(padtrans+k+(i+2)*nRow), _mm_loadu_ps(padtrans+k+(j+2)*nRow)));
                    localsum4b = _mm_add_ps(localsum4b, _mm_mul_ps(*(__m128*)(padtrans+k+(i+3)*nRow), _mm_loadu_ps(padtrans+k+(j+2)*nRow)));
                    /*localsum1c = _mm_add_ps(localsum1c, _mm_mul_ps(*(__m128*)(padtrans+k+i*nRow), *(__m128*)(padtrans+k+(j+3)*nRow)));
                    localsum2c = _mm_add_ps(localsum2c, _mm_mul_ps(*(__m128*)(padtrans+k+(i+1)*nRow), *(__m128*)(padtrans+k+(j+3)*nRow)));
                    localsum3c = _mm_add_ps(localsum3c, _mm_mul_ps(*(__m128*)(padtrans+k+(i+2)*nRow), *(__m128*)(padtrans+k+(j+3)*nRow)));
                    localsum4c = _mm_add_ps(localsum4c, _mm_mul_ps(*(__m128*)(padtrans+k+(i+3)*nRow), *(__m128*)(padtrans+k+(j+3)*nRow)));
				*/
					 }
                ...
                *(C+i+3+j*m) = t3[0]+t3[1]+t3[2]+t3[3];
				...
				/*
				if (j+2 < m) {
                    _mm_storeu_ps(t0,localsum1b);
                    _mm_storeu_ps(t1,localsum2b);
                    _mm_storeu_ps(t2,localsum3b);
                    _mm_storeu_ps(t3,localsum4b);
				...
				/*
				if (j+3 < m) {
                    _mm_storeu_ps(t0,localsum1c);
                    _mm_storeu_ps(t1,localsum2c);
                    _mm_storeu_ps(t2,localsum3c);
                    _mm_storeu_ps(t3,localsum4c);
