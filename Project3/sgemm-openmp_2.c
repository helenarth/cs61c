    ...
    {   
		...
		for (j = 0; j < jroll_limit; j+=JROLL) {  
			for (i = 0; i < roll_limit; i+=ROWWIDTH) {
				...
				_mm_storeu_ps(p1,matrixTrans2);
				_mm_storeu_ps(p2,matrixTrans3);
				...
				*(C+i+2+j*m) = t2[0]+t2[1]+t2[2]+t2[3];
				if (j+ 1 < m) { 
					...
					_mm_storeu_ps(p1,matrixTrans2a);
					_mm_storeu_ps(p2,matrixTrans3a);
					...
					*(C+i+2+(j+1)*m) = t2[0]+t2[1]+t2[2]+t2[3];
					if (j+2 < m) {
						...
						_mm_storeu_ps(p1,matrixTrans2b);
						_mm_storeu_ps(p2,matrixTrans3b);
			...
			for (;i < m;i++) {
				...
				*(C+i+j*m) = t[0]+t[1]+t[2]+t[3];
				if (j+1 < m) {
					_mm_storeu_ps(p1,matrixTrans1a);
					*(C+i+(j+1)*m) = t1[0]+t1[1]+t1[2]+t1[3];       
					if (j+2 < m) {
						_mm_storeu_ps(p2,matrixTrans1b);
